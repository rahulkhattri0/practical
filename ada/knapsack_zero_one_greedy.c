...
{
float weight[50],profit[50],ratio[50],Totalvalue,temp,capacity,amount; int n,i,j;
...
for (i = 0; i < n; i++)
for (j = i + 1; j < n; j++) if (ratio[i] < ratio[j])
{
...
Totalvalue = Totalvalue + profit[i]; capacity = capacity - weight[i];
}
}
if (i < n)
Totalvalue = Totalvalue + (ratio[i]*capacity); printf("\nThe maximum value is :%f\n",Totalvalue); return 0;
}
